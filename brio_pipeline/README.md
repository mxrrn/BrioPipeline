...
./train_classifier.sh
```
Trains MobileNetV3-small on the isolated component dataset (~40 epochs, ~10 min on GPU). Weights are saved to `brio_3d_pipeline/classification/component_classifier.pth` and loaded automatically by the slow pipeline. Only needs to run once; retrain if the component dataset changes.
...
└── brio_pipeline/              # Both pipelines + launchers
    ...
    │   ├── visualize_sam_points.py
    │   │
    │   ├── detection/           # Localize instances in 3D (no class labels)
    │   │   ├── preprocessor.py
    │   │   ├── dust3r_runner.py
    │   │   ├── sam_runner.py       # CLAHE pre-processing + SAM
    │   │   └── backprojector.py    # Back-projection, clustering, visual votes
    │   │
    │   ├── classification/      # Assign class labels to detected instances
    ...
    │   │   └── training/           # One-vs-all dataset builder (Mapping.drawio → 3D → crops)
    │   │       ├── mapping_parser.py
    │   │       ├── dataset_builder.py
    │   │       ├── batch_build.py
    │   │       └── dataset_crops/
...
**File:** `detection/preprocessor.py`
...
**File:** `detection/dust3r_runner.py`  
...
**File:** `detection/backprojector.py`
...
**File:** `classification/component_classifier.py`  
...
The classifier is trained once with `./train_classifier.sh` and its weights (`classification/component_classifier.pth`) are loaded automatically by the pipeline at startup.
...
| `CLASSIFIER_WEIGHTS` | `classification/component_classifier.pth` | MobileNetV3-small checkpoint |
